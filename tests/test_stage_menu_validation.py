...
def test_stage_menu_validation_stub(tmp_path: Path) -> None:
    ...
    validation = menu_validation_run(
        tmp_path / "nav.json", tmp_path / "menu_map.json", tmp_path
    )
    assert (tmp_path / "menu_validation.json").is_file()
    assert validation.model_dump(mode="json") == load_expected_json(
